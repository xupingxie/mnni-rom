...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  1 12:41:44 2018
...
from multistep_PECE import PECE_NN
...
filename = 'Data/2Dcylinder.mat' #load data
...
d = 8   #set input dimension r=8, 
neural = 128   #set the number of unit at hidden layer
layers = [d, neural, d]  # set layers =[input, hidden layer,  output] 
...
# define the NN model
model = PECE_NN(dt, OneXtrain, layers, M)
...
filenamesave = 'Data/PECE2D' +str(L)+'layers'+ str(neural)+'units_r'+str(d)+scheme+'step'+str(M)+'noise'+str(noise)+'.mat'
